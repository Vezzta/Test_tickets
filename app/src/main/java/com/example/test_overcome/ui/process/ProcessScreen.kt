package com.example.test_overcome.ui.process
...
@Composable
fun ProcessScreen(navController: NavController, viewModel: HomeViewModel = hiltViewModel()) {
    ...
    Screen(navController = navController, title = R.string.progress_tickets_title) {
        HomeContent(tickets.filter { it?.status == Status.IN_PROGRESS }, navController)
    }
