package com.example.test_overcome.ui.finished
...
@Composable
fun FinishedScreen(navController: NavController, viewModel: HomeViewModel = hiltViewModel()) {
    ...
    Screen(navController = navController, title = R.string.finished_tickets_title) {
        HomeContent(tickets.filter { it?.status == Status.FINISHED }, navController)
    }
